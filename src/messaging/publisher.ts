...
export class Publisher extends MessagingBus {
    ...
    public async initialize() {
        ...
        this.logger.debug({action: 'INITIALISE', MODE: 'PUBLISHER', status: 'STARTING'})
        ...
        this.logger.info({action: 'INITIALISE',  MODE: 'PUBLISHER',status: 'COMPLETE'})
    }

    public async publish(message: SmartlyEvent) {
        await this.initialize()
        this.channel?.publish(this.#outboundExchangeName, '', Buffer.from(message.toBuffer()))
